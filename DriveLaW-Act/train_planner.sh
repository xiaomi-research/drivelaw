...
torchrun $DISTRIBUTED_ARGS \
    navsim/agents/videodrive/run_training_videodrive.py \
    agent=videodrive_agent \
    agent.config_file="navsim/agents/videodrive/configs/ltx_model/video_model_infer_navsim.yaml" \
    experiment_name=training_video_agent \
    train_test_split=$TRAIN_TEST_SPLIT \
    cache_path="${CACHE_PATH:-./cache/recogdrive_agent_cache_dir_train}" \
    use_cache_without_dataset=False > train_drivelaw_act.txt 2>&1