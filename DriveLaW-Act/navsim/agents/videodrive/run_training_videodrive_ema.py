...
# ===========================
# Prompt builder (as per your rules)
# ===========================
...
def _resize_to_hw(img: torch.Tensor, height: int = 704, width: int = 1280) -> torch.Tensor:
    ...
    Resizes to fixed (height, width); returns directly if already at this size.
    """
    if img.dim() == 3:  # (C,H,W)
        _, H, W = img.shape
        if (H, W) == (height, width):
...
def build_prompt_fixed(hist_xyh: torch.Tensor,
    ...
    """
    Args:
        hist_xyh: (T_hist, 3) Last frame ~ (0,0,0), first frame is earliest history
...
class CacheOnlyDatasetLazyIO(CacheOnlyDataset):
    ...
    Allows two forms to coexist in cache:
      1) features["images"] is already (T,C,H,W) Tensor -- returned directly
      2) features["image_paths"] is List[str|Path] -- lazily loaded as Tensor in __getitem__
    Similarly for targets:
      - targets["future_frames"] is directly (Tf,C,H,W)
      - Or targets["future_paths"] is List[str|Path], lazily loaded
    """
    ...
    def __init__(
        ...
        *,
        normalize: str = "[-1,1]",
        min_hist_frames: int | None = None,   # Optional: pad by repeating segments if shorter than this length (e.g., 8)
        min_fut_frames: int | None = None,    # Optional: minimum future frames length (pad if needed)
    ):
    ...
    def _load_one_image(self, p: Union[str, Path]) -> torch.Tensor:
        p = Path(p)
        # Uniformly read with PIL, then convert to RGB
    ...
    def _maybe_load_images_from_paths(self, maybe_paths: Any) -> torch.Tensor:
        """
        Supports:
          - Already a Tensor: returned directly
          - List[str|Path]: lazily loaded -> (T,C,H,W) Tensor
        """
        ...
        # If empty, return an empty Tensor as placeholder
...
# =========================================
# collate: NavSim -> batch required for training loop
# =========================================
...
def navsim_genie_collate_fn(
    ...
    Outputs:
      ...
      future_video  : (B, C, 1, T_fut, H, W) -- returned only if all in batch have future_frames
    """
    ...
    # --------- New: also return metadata in batch ----------
...
def _pad_to_8n1(x_5d: torch.Tensor) -> torch.Tensor:
    ...
    """
    T = x_5d.shape[2]
    need = (1 - (T % 8)) % 8  # such that (T + need) % 8 == 1
    if need > 0:
...
def _to_8n1(video_5d: torch.Tensor, ctx_5d: torch.Tensor = None):
    ...
    if ctx_5d is not None:
        ...
        video_5d = torch.cat([ctx_5d, video_5d], dim=2)   # First add 1 context frame

    T = video_5d.shape[2]
    r = (1 - (T % 8)) % 8            # Number of frames to pad, such that (T + r) % 8 == 1
    if r > 0:
...
# ===========================
# Unified Trainer (Rewritten Version)
# ===========================
class State:
    seed: int = None
...
class UnifiedTrainer:
    """
    - Data: From NavSim (SceneLoader/Dataset built within prepare_dataset/prepare_val_dataset)
    - Model/Training Loop: Follows your previous diffusion training logic (video/action branches, flow matching, accelerate)
    - Text: Caption generated by collate using a fixed template
    """

    def __init__(self, cfg,config_file):
        """
        Args:
            cfg: Your Hydra training configuration, must include:
              ...
              - genie_config: Model/optimization related YAML path (following your genie configuration)
        ...
        self.ema_update_after_step = int(getattr(self.args, "ema_update_after_step", 0))  # 0 means immediate effect
        self.ema_inv_gamma = float(getattr(self.args, "ema_inv_gamma", 1.0))              # warmup curve
        self.ema_power = float(getattr(self.args, "ema_power", 2/3))                      # warmup curve
    ...
    def prepare_trainable_parameters(self):
        ...
        if self.use_ema:
            ...
            # Unified tracked parameter order (only includes requires_grad=True)
            ...
            # Move to correct device (especially useful with ZeRO/offload; harmless if not needed)
            try:
        ...
        for epoch in range(self.state.train_epochs):
            ...
            for step, batch in enumerate(self.train_dataloader):
                ...
                with accel.accumulate(self.diffusion_model):
                    # ---------------------------
                    # 1) Retrieve historical frames and optional future frames
                    # ---------------------------
                    ...
                    mem = video                                                                # Here your input hist is 8 frames
                    ...
                    # ---------------------------
                    # 4) Flow-matching (video side, only for noise/timesteps; video loss may not be calculated)
                    # ---------------------------
                    ...
                    noisy_latents = noise
                    sigmas_vec = self._timestep_sampler.sample_for(noisy_latents)   # <== This line is required by you
                    sigmas = sigmas_vec                                            # (b,)
                    ...
                    # >>> EMA update (only on sync gradient steps, to avoid intermediate noise from gradient accumulation) <<<
                    if self.use_ema and accel.sync_gradients:
                        base = unwrap_model(accel, self.diffusion_model)
                        ema_params_now = [p for p in base.parameters() if p.requires_grad]  # Consistent with initialization set
    ...
    def _forward_pass(self, latents, timesteps, prompt_embeds, prompt_attention_mask,
        ...
        """Thin wrapper, compatible with your utils.forward_pass interface/return."""
        return forward_pass(
    ...
    def validate(self, accelerator, model_save_dir, global_step):
        """
        Performs action validation using the current NavSim collate format:
        - Conditioning input: all historical frames from batch['video']
        - Target: batch['actions']
        - Metric: MSE(L2) over the entire validation set
        - Artifacts: saves pred / gt to model_save_dir
        """
        ...
        # Inference base hyperparameters (keep fields from your existing config)
        num_steps   = 5
        ...
        for batch in self.val_dataloader:
            # Historical condition: (B,C,1,T,H,W) -> (B,C,T,H,W)
            ...
            for i in range(B):
                ...
                out = pipe.infer(
                    ...
                    output_type = "latent",
                    return_action=True,
                    return_video=False,          # Only action
                ...
                # Extract predicted actions, unify to (T,C) on cpu
