...
#from diffusers.models import ContextParallelInput, ContextParallelOutput
from diffusers.models.attention import AttentionMixin, AttentionModuleMixin, FeedForward, Attention
...
class LTXVideoTransformer3DModel(
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if self.action_scorer:
            self.traj_scorer = ActionTrajectoryScorer()
    ...
    def forward(
        ...
        if return_action:
            ...
            action_temb_fused, action_embedded_timestep, action_rotary_emb, action_hidden_states = preprocessing_action_states(self, action_states, action_timestep,context_dict)
        ...
        for block_idx, block in enumerate(self.transformer_blocks):
            if torch.is_grad_enabled() and self.gradient_checkpointing:
                ...
                if return_action:
                    ...
                    assert B_action % B_video == 0, "动作与视频 batch 尺寸不对齐"
                    K = B_action // B_video

                    final_hidden_states = hidden_states.repeat_interleave(K, dim=0)  # (B*K, S, D)
        ...
        if return_action:
            ...
            action_output = self.action_proj_out(action_hidden_norm)

            # === 分类 logit（每个 mode 一个标量）：先时序平均，再线性到 1 ===
            pooled = action_hidden_norm.mean(dim=1)            # (B*K, D)
            logits = self.action_cls_head(pooled)              # (B*K, 1)

            final_output['action'] = action_output             # (B*K, T, 3)
            final_output['action_logits'] = logits             # (B*K, 1)
