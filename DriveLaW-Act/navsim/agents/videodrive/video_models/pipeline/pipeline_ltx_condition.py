...
class LTXConditionPipeline(DiffusionPipeline, FromSingleFileMixin, LTXVideoLoraLoaderMixin):
    ...
    def _encode_per_frame_to_latents4d(self, data, generator=None, device=None, dtype=torch.float32):
        """
        data: (B, C, F, H, W)
        return: (B, C_lat, F, H_l, W_l)  逐帧编码（F 不再被 VAE 压缩）
        """
        device = device or data.device
        B, C, F, H, W = data.shape
        x = data.permute(0, 2, 1, 3, 4).reshape(B * F, C, 1, H, W)   # (B*F, C, 1, H, W)

        z = retrieve_latents(self.vae.encode(x), generator=generator) # (B*F, C_lat, 1, H_l, W_l)
        z = self._normalize_latents(z, self.vae.latents_mean, self.vae.latents_std)
        C_lat, H_l, W_l = z.shape[1], z.shape[-2], z.shape[-1]

        z = z.view(B, F, C_lat, 1, H_l, W_l).transpose(1, 2).squeeze(3).contiguous()  # (B, C_lat, F, H_l, W_l)
    ...
    def prepare_latents(
        ...
        if len(conditions) > 0:
            ...
            for data, strength, frame_index in zip(conditions, condition_strength, condition_frame_index):
                if per_frame_encode and frame_index == 0:
                    # 历史帧：逐帧 encode（F 保持不压缩）
                    condition_latents = self._encode_per_frame_to_latents4d(
                        data, generator=generator, device=device, dtype=dtype
                    )
                    scale_t_this = 1  # 逐帧
                else:
                    # 原逻辑：让 VAE 压缩时间维（F -> (F-1)//ratio+1）
                    ...
                    scale_t_this = self.vae_temporal_compression_ratio

                condition_latents = condition_latents.repeat(num_videos_per_prompt,1,1,1,1)
                Bc, C_lat, F_cond, Hl, Wl = condition_latents.shape

                assert Bc == batch_size and Hl == latent_height and Wl == latent_width

                if frame_index == 0:
                    # 直接贴到主 latent 前部；此时 num_latent_frames= M(逐帧) + F_cond(压缩后)
                    F_use = min(F_cond, num_latent_frames)
                    if abs(strength - 1.0) < 1e-6:
                        latents[:, :, :F_use] = condition_latents[:, :, :F_use]
                    else:
                        latents[:, :, :F_use] = torch.lerp(latents[:, :, :F_use], condition_latents[:, :, :F_use], strength)
                    condition_latent_frames_mask[:, :F_use] = strength
                    # 注意：这里无需构造额外 token，直接占据主序列的前 M/F_use 个槽位
                else:
                    # 原来的 prefix / extra token 逻辑不变
                    if F_cond < num_prefix_latent_frames:
                        raise ValueError(f"Need at least {num_prefix_latent_frames} latent frames but got {F_cond}")

                    prefix = num_prefix_latent_frames
                    start  = frame_index // scale_t_this + prefix
                    end    = min(start + (F_cond - prefix), num_latent_frames)
                    if start < num_latent_frames and end > start:
                        latents[:, :, start:end] = torch.lerp(
                            latents[:, :, start:end],
                            condition_latents[:, :, prefix:prefix + (end - start)],
                            strength,
                        )
                        condition_latent_frames_mask[:, start:end] = strength

                    # prefix 部分作为“额外条件 token”附加（与原来一致）
                    keep_prefix = condition_latents[:, :, :prefix]
                    noise_pf = randn_tensor(keep_prefix.shape, generator=generator, device=device, dtype=dtype)
                    keep_prefix = torch.lerp(noise_pf, keep_prefix, strength)

                    cond_ids = self._prepare_video_ids(
                        batch_size, prefix, latent_height, latent_width,
                        patch_size=self.transformer_spatial_patch_size,
                        patch_size_t=self.transformer_temporal_patch_size,
                        device=device,
                    )
                    cond_ids = self._scale_video_ids(
                        cond_ids,
                        scale_factor=self.vae_spatial_compression_ratio,
                        scale_factor_t=scale_t_this,      # 逐帧→1；压缩→temporal_ratio
                        frame_index=frame_index,
                        device=device,
                    )
                    keep_prefix_tok = self._pack_latents(
                        keep_prefix, self.transformer_spatial_patch_size, self.transformer_temporal_patch_size
                    )
                    cond_mask = torch.full(keep_prefix_tok.shape[:2], strength, device=device, dtype=dtype)

                    extra_conditioning_latents.append(keep_prefix_tok)
                    extra_conditioning_video_ids.append(cond_ids)
                    extra_conditioning_mask.append(cond_mask)
                    extra_conditioning_num_latents += keep_prefix_tok.size(1)
    ...
    def infer(
        ...
        action_timestep: Optional[torch.LongTensor] = None,
        **kwargs,
    ):
        r"""
        ...
        if is_conditioning_image_or_video:
            for condition_image, condition_video, condition_frame_index, condition_strength in zip(
                ...
                use_per_frame = (condition_frame_index == 0)  
                if condition_image is not None:
                    ...
                    if not use_per_frame:
                        ...
                        num_frames_output = self.trim_conditioning_sequence(condition_frame_index, num_frames_input, num_frames)
                ...
                if (not use_per_frame) and (condition_tensor.size(2) % self.vae_temporal_compression_ratio != 1):
                    raise ValueError(
        ...
        K = num_videos_per_prompt
        action_bs = batch_size * K
        ...
        if return_action:
            ...
            actions = randn_tensor((action_bs, action_chunk, action_dim), device=device, dtype=prompt_embeds.dtype, generator=action_generator)
        else:
            actions = None
        ...
        NV = getattr(self.scheduler.config,        "num_train_timesteps", 1000)
        NA = getattr(self.scheduler_action.config, "num_train_timesteps", NV)

        if timesteps is None:
            sigmas = linear_quadratic_schedule(num_inference_steps)
            timesteps = sigmas * 1000
        timesteps_a_idx = timesteps
        timesteps, num_inference_steps = retrieve_timesteps(self.scheduler, num_inference_steps, device, timesteps)
        timesteps_a, _                 = retrieve_timesteps(self.scheduler_action, num_inference_steps, device, timesteps_a_idx)
        ...
        latents, conditioning_mask, video_coords, extra_conditioning_num_latents = self.prepare_latents(
            ...
            height=height,
            width=width,
            num_latent_frames=main_num_latent_frames,
            sigma=latent_sigma,
            latents=latents,
            generator=generator,
            device=device,
        ...
        def _dup_if_cfg(x, do_cfg: bool):
            if x is None:
                return None
            return torch.cat([x, x], dim=0) if do_cfg else x

        def _repeat_multi_modal(x,num):
            if x is None:
                return None
            x = x.repeat(num,1)
            return x

        context_dict = {k: _repeat_multi_modal(v, num_videos_per_prompt) for k, v in context_dict.items()}

        if self.do_classifier_free_guidance:
            video_coords = torch.cat([video_coords, video_coords], dim=0)
            if context_dict is not None:
                context_dict = {k: _dup_if_cfg(v, True) for k, v in context_dict.items()}
        
        # 6. Denoising loop
        video_states_buffer = None
        for i, (t, t_a) in enumerate(zip(timesteps, timesteps_a)):
                if self.interrupt:
                    continue
                ...
                timestep = t.expand(latent_model_input.shape[0], conditioning_mask_model_input.shape[1]).float()
                if is_conditioning_image_or_video:
                    cond_bin = (conditioning_mask_model_input > 0.5).float()   # 二值化成 0/1
                    timestep = timestep * (1.0 - cond_bin)                     # 条件 token 直接置 0

                if return_action:
                    # (B, action_chunk)
                    action_t_int = t_a.round().long()                            # ( ) 标量
                    action_timesteps = action_t_int.expand(action_bs, action_chunk)  # (B,L)
                    ...
                    if self.do_classifier_free_guidance:
                        ...
                        action_timesteps = torch.cat([action_timesteps, action_timesteps], dim=0)
                        history_action_state_in = (
                            torch.cat([history_action_state, history_action_state], dim=0)
                            if history_action_state is not None else None
                        )    
                    else:
                        actions_in = actions
                        history_action_state_in = history_action_state
                    actions_in = actions_in.to(prompt_embeds.dtype)
                    if history_action_state_in is not None:
                ...
                with self.transformer.cache_context("cond_uncond"):
                    noise_pred = self.transformer(
                        ...
                        num_frames=main_num_latent_frames,
                ...
                if return_action:
                    ...
                    actions = self.scheduler_action.step(action_noise_pred, t_a, actions, return_dict=False)[0]
                ...
                if compute_video:
                    ...
                    if self.do_classifier_free_guidance:
                        ...
                        if self.guidance_rescale > 0:
                            video_noise_pred = rescale_noise_cfg(
                                video_noise_pred, video_noise_pred_text, guidance_rescale=self.guidance_rescale
                            )
                    denoised_latents = self.scheduler.step(
                        video_noise_pred, t, latents, per_token_timesteps=timestep, return_dict=False
                    )[0]

                    if i == i_ctx:
                        ctx_tokens = denoised_latents.detach() 
        ...
        if return_action:
            if return_dict:
                return {"actions": actions,"videos": video}
            else:
