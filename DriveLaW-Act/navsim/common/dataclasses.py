        ...
        for camera_name in camera_dict.keys():
            ...
            if camera_identifier in sensor_names:
                ...
                if not load_image_path:
                    if camera_identifier == 'cam_f0':
                        print(image_path)
                    try:
                        img_np = np.array(Image.open(image_path).convert("RGB"))
                    except Exception:
                        alt_path = _reroot_path(image_path)
                        img_np = np.array(Image.open(alt_path).convert("RGB"))
                        image_path = alt_path  
...
class Scene:
    ...
    def get_future_frames(self, num_trajectory_frames: Optional[int] = None):
        """
        连续取未来的 cameras 列表，起点为 num_history_frames。
        返回: List[Cameras]
        """
        if num_trajectory_frames is None:
            num_trajectory_frames = self.scene_metadata.num_future_frames
        ...
        end_idx = start_idx + num_trajectory_frames

        return [self.frames[i].cameras for i in range(start_idx, end_idx)]
