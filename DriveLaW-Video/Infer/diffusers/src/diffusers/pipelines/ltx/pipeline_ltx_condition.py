...
class LTXConditionPipeline(DiffusionPipeline, FromSingleFileMixin, LTXVideoLoraLoaderMixin):
    ...
    def _compute_high_frequency_mask(
        self,
        video_frames: torch.Tensor,
        beta: float = 1.0,
    ) -> torch.Tensor:
        """
        Compute high-frequency mask from video frames in pixel space.
        
        Args:
            video_frames: Video tensor of shape [B, C, F, H, W] in pixel space
            beta: Threshold coefficient for adaptive thresholding
            
        Returns:
            High-frequency mask of shape [B, F, H, W]
        """
        # Convert to grayscale: [B, C, F, H, W] -> [B, F, H, W]
        # Use standard RGB to grayscale conversion weights
        if video_frames.shape[1] == 3:  # RGB
            grayscale = (
                0.299 * video_frames[:, 0] + 
                0.587 * video_frames[:, 1] + 
                0.114 * video_frames[:, 2]
            )
        else:
            # If already grayscale or single channel, take first channel
            grayscale = video_frames[:, 0]
        
        B, num_frames, H, W = grayscale.shape
        masks = []
        ...
        # Apply Laplacian filter to each frame independently
        for b in range(B):
            batch_masks = []
            for f in range(num_frames):
                frame = grayscale[b, f].unsqueeze(0).unsqueeze(0)  # [1, 1, H, W]
                # Apply Laplacian filter
                laplacian_response = F.conv2d(frame, laplacian_kernel, padding=1)  # [1, 1, H, W]
                laplacian_response = laplacian_response.squeeze()  # [H, W]
                
                # Compute absolute value of response
                Hf = torch.abs(laplacian_response)
                
                # Compute adaptive threshold: au = β * std(Hf)
                au = beta * torch.std(Hf)
                
                # Create binary mask: Mf(x, y) = 1 if Hf(x, y) > au, else 0
                mask = (Hf > au).float()
                batch_masks.append(mask)
            
            masks.append(torch.stack(batch_masks, dim=0))  # [num_frames, H, W]
        
        mask_tensor = torch.stack(masks, dim=0)  # [B, num_frames, H, W]
        return mask_tensor

    def _apply_noise_reinjection(
        self,
        latents: torch.Tensor,
        t: float,
        ...
        height: int,
        width: int,
        extra_conditioning_num_latents: int,
        is_conditioning_image_or_video: bool,
        attention_kwargs: Optional[Dict[str, Any]],
        do_classifier_free_guidance: bool,
        generator: Optional[torch.Generator],
        beta: float = 1.0,
        sigma_prime: float = 0.1,
    ) -> torch.Tensor:
        """
        Apply noise reinjection to high-frequency regions.
        
        Args:
            latents: Current packed latents [B, L, D]
            t: Current timestep
            prompt_embeds: Prompt embeddings
            prompt_attention_mask: Prompt attention mask
            video_coords: Video coordinates
            num_frames: Number of latent frames
            latent_height: Latent height
            latent_width: Latent width
            height: Pixel height
            width: Pixel width
            extra_conditioning_num_latents: Number of extra conditioning latents
            is_conditioning_image_or_video: Whether using image/video conditioning
            attention_kwargs: Attention kwargs
            do_classifier_free_guidance: Whether using classifier-free guidance
            generator: Random generator
            beta: Threshold coefficient for mask computation
            sigma_prime: Noise strength for reinjection
            
        Returns:
            Perturbed latents with noise reinjection [B, L, D]
        """
        # Extract main latents (remove extra conditioning if present)
        # Note: video_coords may already be duplicated for classifier-free guidance
        # Check if video_coords batch size matches latents batch size
        video_coords_batch_size = video_coords.shape[0]
        latents_batch_size = latents.shape[0]
        video_coords_already_duplicated = (video_coords_batch_size == latents_batch_size * 2)
        
        # Extract main latents and corresponding video_coords
        if is_conditioning_image_or_video and extra_conditioning_num_latents > 0:
            main_latents = latents[:, extra_conditioning_num_latents:]
            # video_coords shape is [B_or_2B, 3, L_total], extract main part
            # Ensure we're extracting from the correct dimension (last dimension is sequence length)
            main_video_coords = video_coords[:, :, extra_conditioning_num_latents:]
        else:
            main_latents = latents
            main_video_coords = video_coords
        
        # Verify dimensions match
        main_latents_seq_len = main_latents.shape[1]  # [B, L, D]
        main_video_coords_seq_len = main_video_coords.shape[2]  # [B_or_2B, 3, L]
        if main_latents_seq_len != main_video_coords_seq_len:
            raise ValueError(
                f"Sequence length mismatch: main_latents has {main_latents_seq_len} tokens, "
                f"but main_video_coords has {main_video_coords_seq_len} tokens"
            )
        
        # Prepare inputs for transformer prediction (need packed format)
        # Handle batch dimension: ensure latents and video_coords have matching batch size
        latent_model_input = main_latents.to(prompt_embeds.dtype)
        
        if video_coords_already_duplicated:
            # video_coords is already duplicated (for CFG), so duplicate latents to match
            latent_model_input = torch.cat([latent_model_input] * 2)
        elif do_classifier_free_guidance:
            # video_coords is not duplicated yet, but we need CFG, so duplicate both
            latent_model_input = torch.cat([latent_model_input] * 2)
            main_video_coords = torch.cat([main_video_coords, main_video_coords], dim=0)
        
        # Final dimension check before transformer call
        assert latent_model_input.shape[0] == main_video_coords.shape[0], \
            f"Batch size mismatch: latent_model_input={latent_model_input.shape[0]}, main_video_coords={main_video_coords.shape[0]}"
        assert latent_model_input.shape[1] == main_video_coords.shape[2], \
            f"Sequence length mismatch: latent_model_input={latent_model_input.shape[1]}, main_video_coords={main_video_coords.shape[2]}"
        
        timestep_tensor = t.expand(latent_model_input.shape[0]).unsqueeze(-1).float()
        
        # Initial prediction: L̂₀ = Ψθ(Lt, t) (using packed format)
        with self.transformer.cache_context("cond_uncond"):
            noise_pred = self.transformer(
                ...
                video_coords=main_video_coords,
                attention_kwargs=attention_kwargs,
                return_dict=False,
            )[0]
        
        # Handle classifier-free guidance if used
        if do_classifier_free_guidance:
            noise_pred_uncond, noise_pred_text = noise_pred.chunk(2)
            noise_pred = noise_pred_uncond + self.guidance_scale * (noise_pred_text - noise_pred_uncond)
            timestep_tensor = timestep_tensor.chunk(2)[0]
        
        # Get predicted clean latent (approximate L̂₀)
        # Use a simplified formula to avoid scheduler.step index issues at the last timestep
        # For Flow Matching: x_0 ≈ x_t - t * v_t, where v_t = -noise_pred
        # We convert timestep t to normalized time (0-1 range)
        t_normalized = t / 1000.0  # Convert from [0, 1000] to [0, 1]
        # Approximate clean latent: L̂₀ ≈ L_t - t * (-noise_pred) = L_t + t * noise_pred
        predicted_latents_packed = main_latents + t_normalized * noise_pred
        
        # Unpack predicted latents for VAE decoding
        predicted_latents = self._unpack_latents(
            predicted_latents_packed,
            num_frames,
            latent_height,
            latent_width,
            self.transformer_spatial_patch_size,
        ...
        # Denormalize predicted latents for VAE decoding
        # Note: predicted_latents from scheduler are already normalized,
        # so we just need to denormalize them for VAE
        predicted_latents_for_vae = self._denormalize_latents(
            predicted_latents,
        ...
        # Decode to pixel space: Î₀ = D(L̂₀)
        # Use timestep 0.0 for clean prediction
        decode_timestep = torch.zeros(
            predicted_latents_for_vae.shape[0],
            device=predicted_latents_for_vae.device,
            dtype=predicted_latents_for_vae.dtype,
        )
        pixel_frames = self.vae.decode(
            predicted_latents_for_vae.to(self.vae.dtype),
            decode_timestep,
            return_dict=False,
        )[0]  # [B, C, F, H, W] in pixel space
        
        # Compute high-frequency mask in pixel space
        pixel_mask = self._compute_high_frequency_mask(pixel_frames, beta=beta)  # [B, F, H, W]
        
        # Downsample mask from pixel resolution to latent resolution using nearest-neighbor
        # pixel_mask: [B, F, H, W]
        # target: [B, F, latent_height, latent_width]
        pixel_mask_4d = pixel_mask.unsqueeze(1)  # [B, 1, F, H, W] for interpolation
        ...
        # Unpack original latents for noise injection
        original_unpacked_latents = self._unpack_latents(
            main_latents,
            num_frames,
            latent_height,
            latent_width,
        ...
        # Generate noise for reinjection
        noise_shape = original_unpacked_latents.shape  # [B, C, F, H, W]
        reinjection_noise = randn_tensor(
            noise_shape,
            generator=generator,
            device=original_unpacked_latents.device,
            dtype=original_unpacked_latents.dtype,
        )
        
        # Apply mask: expand mask from [B, F, H, W] to [B, C, F, H, W]
        latent_mask_expanded = latent_mask.unsqueeze(1)  # [B, 1, F, H, W]
        
        # Inject noise: L'ₜ = Lₜ + σ'ₜ · M ⊙ εₜ
        # Apply noise to original latents (Lt), not predicted latents
        perturbed_latents = original_unpacked_latents + sigma_prime * latent_mask_expanded * reinjection_noise
        
        # Repack latents from [B, C, F, H, W] to [B, L, D]
        packed_perturbed_latents = self._pack_latents(
            perturbed_latents,
            self.transformer_spatial_patch_size,
            self.transformer_temporal_patch_size,
        )
        
        # Reattach extra conditioning latents if they were present
        if is_conditioning_image_or_video and extra_conditioning_num_latents > 0:
            extra_latents = latents[:, :extra_conditioning_num_latents]
            packed_perturbed_latents = torch.cat([extra_latents, packed_perturbed_latents], dim=1)
        
        return packed_perturbed_latents
    ...
    def __call__(
        ...
        noise_reinjection_enabled: bool = False,
        noise_reinjection_beta: float = 1.0,
        noise_reinjection_sigma: float = 0.1,
        noise_reinjection_steps: Optional[int] = None,
        ...
        Args:
            ...
            image_cond_noise_scale (`float`, *optional*, defaults to 0.15):
                The scale of noise added to image conditioning latents at each timestep.
            noise_reinjection_enabled (`bool`, *optional*, defaults to `False`):
                Whether to enable noise reinjection for high-frequency detail preservation. When enabled, selectively
                re-injects noise into high-frequency regions before each denoising step to prevent over-smoothing and
                preserve fine textures, especially important for high-speed driving video generation.
            noise_reinjection_beta (`float`, *optional*, defaults to 1.0):
                Threshold coefficient β for adaptive high-frequency mask computation. The adaptive threshold is computed
                as `β * std(Hf)` where Hf is the Laplacian response map. Higher values result in more conservative
                masking (fewer regions selected for noise reinjection).
            noise_reinjection_sigma (`float`, *optional*, defaults to 0.1):
                Noise strength σ'ₜ for the reinjection step. Controls the amount of noise added to high-frequency
                regions. Higher values lead to stronger noise reinjection.
            noise_reinjection_steps (`int`, *optional*, defaults to `None`):
                Number of initial steps to apply noise reinjection. If `None` and `noise_reinjection_enabled=True`,
                noise reinjection will be applied to all steps. If set to a positive integer, noise reinjection will
                only be applied to the first N steps. This is useful as noise reinjection is typically more beneficial
                in early denoising steps when there is more noise to refine.
            num_videos_per_prompt (`int`, *optional*, defaults to 1):
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                # Apply noise reinjection if enabled and within the specified step range
                should_apply_reinjection = noise_reinjection_enabled
                if should_apply_reinjection and noise_reinjection_steps is not None:
                    # Only apply in the first N steps
                    should_apply_reinjection = (i < noise_reinjection_steps)
                
                if should_apply_reinjection:
                    latents = self._apply_noise_reinjection(
                        latents=latents,
                        t=t,
                        prompt_embeds=prompt_embeds,
                        prompt_attention_mask=prompt_attention_mask,
                        ...
                        height=height,
                        width=width,
                        extra_conditioning_num_latents=extra_conditioning_num_latents,
                        is_conditioning_image_or_video=is_conditioning_image_or_video,
                        attention_kwargs=attention_kwargs,
                        do_classifier_free_guidance=self.do_classifier_free_guidance,
                        generator=generator,
                        beta=noise_reinjection_beta,
                        sigma_prime=noise_reinjection_sigma,
                    )
