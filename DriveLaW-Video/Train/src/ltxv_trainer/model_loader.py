...
def load_scheduler(model_source: ModelSource | None = None) -> FlowMatchEulerDiscreteScheduler:
    """
    Load the Flow Matching scheduler component from the specified source.

    Args:
        model_source: Model source (HF repo, local path, or version). If None, uses HF_MAIN_REPO.
    ...
    if model_source is None:
        source = HF_MAIN_REPO
    elif isinstance(model_source, LtxvModelVersion):
        try:
            source = model_source.hf_repo
        except ValueError:
            # For versions without HF repo, use main repo
            source = HF_MAIN_REPO
    elif isinstance(model_source, (str, Path)):
...
def load_tokenizer(model_source: ModelSource | None = None) -> T5Tokenizer:
    """
    Load the T5 tokenizer component from the specified source.
...
def load_text_encoder(model_source: ModelSource | None = None, *, load_in_8bit: bool = False) -> T5EncoderModel:
    """
    Load the T5 text encoder component from the specified source.
    ...
    return T5EncoderModel.from_pretrained(source, subfolder="text_encoder", **kwargs)
...
def load_ltxv_components(
    ...
    *,
    ...
    Args:
        model_source: Source to load all components from. Can be:
            - HuggingFace repo ID (e.g. "Lightricks/LTX-Video")
            - Local path to model files (str or Path)
            - LtxvModelVersion enum value
            - None (will use the latest version and HF_MAIN_REPO for scheduler/tokenizer/text_encoder)
    ...
    return LtxvModelComponents(
        ...
        text_encoder=load_text_encoder(model_source, load_in_8bit=load_text_encoder_in_8bit),
